...
export class TimeStep {
    ...
    updateScale(
        ...
        minimumStep: number = this.minimumStep
    ) {
        this.left = start;
        this.right = end;
            ...
            case 'hour':
                ...
                if (this.current.month() < 6) {
                    this.current = this.current.subtract(
                        this.current.hour() % this.step,
                        'hour'
                    );
                } else {
                    if (this.current.hour() % this.step !== 0) {
                        this.current = this.current.add(
                            this.step - (this.current.hour() % this.step),
                            'hour'
                        );
                    }
                }
            ...
            switch (this.scale) {
                case 'millisecond':
                    if (
                        ...
                        this.current.millisecond() < this.step
                    )
    ...
    roundToMinor() {
        ...
        switch (this.scale) {
            case 'year':
                this.current = this.current.year(
            ...
            case 'weekday':
                this.current = this.current.hour(0); // eslint-disable-line no-fallthrough
            case 'hour':
                this.current = this.current.minute(0); // eslint-disable-line no-fallthrough
            case 'minute':
                this.current = this.current.second(0); // eslint-disable-line no-fallthrough
            case 'second':
                this.current = this.current.millisecond(0); // eslint-disable-line no-fallthrough
        ...
        if (this.step != 1) {
            ...
            switch (this.scale) {
                ...
                case 'second':
                    this.current = this.current.subtract(
                        this.current.second() % this.step,
                        'seconds'
                    );
                    break;
                case 'minute':
                    this.current = this.current.subtract(
                        this.current.minute() % this.step,
                        'minutes'
                    );
                    break;
                case 'hour':
    ...
    static snap(date: any, scale: string, step: number) {
        ...
        if (scale == 'year') {
            ...
            switch (step) {
                case 5:
                case 2:
                    clone.hour(Math.round(clone.hour() / 24) * 24);
                    break;
                default:
                    clone.hour(Math.round(clone.hour() / 12) * 12);
                    break;
            }
            clone.minute(0);
            clone.second(0);
            ...
            switch (step) {
                ...
                case 2:
                    ...
                    break;
                default:
                    clone.hour(Math.round(clone.hour() / 6) * 6);
                    break;
            }
            clone.minute(0);
            clone.second(0);
            ...
            switch (step) {
                case 4:
                    clone.minute(Math.round(clone.minute() / 60) * 60);
                    break;
                default:
                    clone.minute(Math.round(clone.minute() / 30) * 30);
                    break;
            }
            clone.second(0);
            clone.millisecond(0);
            ...
            switch (step) {
                ...
                case 10:
                    ...
                    break;
                case 5:
                    clone.second(Math.round(clone.second() / 60) * 60);
                    break;
                default:
                    clone.second(Math.round(clone.second() / 30) * 30);
                    break;
            }
            clone.millisecond(0);
            ...
            switch (step) {
                ...
                case 10:
                    ...
                    break;
                case 5:
                    clone.millisecond(Math.round(clone.millisecond() / 1000) * 1000);
                    break;
                default:
                    clone.millisecond(Math.round(clone.millisecond() / 500) * 500);
                    break;
            }
    ...
    getClassName() {
        ...
        switch (this.scale) {
            ...
            case 'hour':
                classNames.push(
                    `mormo-h${current.hour()}${
                        this.step == 4 ? '-h' + (current.hour() + 4) : ''
                    }`
                );
                classNames.push(today(current));
            ...
            case 'day':
                ...
                classNames.push(
                    this.step <= 2 ? `mormo-${current.format('dddd').toLowerCase()}` : ''
                );
